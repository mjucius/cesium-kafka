 ...
 *
 * <p>Layering: {@link events.cesium.kafka.store.tracker.index.EntryPool} (slot storage),
 * {@link events.cesium.kafka.store.tracker.index.DispatchHeap} (lazy-deletion min-heap on
 * {@code dispatchAtMs}), {@link events.cesium.kafka.store.tracker.index.ArrivalLog} (append-only
 * arrival order, doubly sorted, binary-searchable) compose into one
 * {@link events.cesium.kafka.store.tracker.index.PartitionShard} per tracker partition;
 * {@link events.cesium.kafka.store.tracker.index.TrackerIndex} is the multi-shard container with
