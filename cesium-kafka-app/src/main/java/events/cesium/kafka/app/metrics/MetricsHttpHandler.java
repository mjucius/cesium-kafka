 ...
 * registry::scrape}) so the handler stays decoupled from the registry type and from Kafka client
 * ownership — it renders whatever meters are registered, and each loop self-registers its
 * {@code KafkaClientMetrics} binder on its own thread (the {@code EngineHealth}/registry seam).
 *
