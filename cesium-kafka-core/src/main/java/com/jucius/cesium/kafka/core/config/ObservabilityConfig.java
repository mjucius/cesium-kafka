 ...
 * @param bindAddress the interface the listener binds; default {@code 0.0.0.0} (all interfaces, for
 *     k8s probe compatibility). Set {@code 127.0.0.1} to restrict the unauthenticated endpoints to a
 *     loopback sidecar scrape (security L1).
...
public record ObservabilityConfig(Integer port, String bindAddress, Boolean detailedInfo) {
    ...
    /** Default bind address: all interfaces, so k8s liveness/readiness probes reach the listener. */
